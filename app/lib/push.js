...
exports.pushRecieved = function (_data, isClick) {
  ...
  if (!isClick) {
    if (!Alloy.Globals.appOnline) return;

    // display only 1 miniute before.
    var now = new Date();
    ...
    if (intervalMin < 1) {
      ...
      dialog.addEventListener('click', function (e){
        ...
        if (e.index === e.source.cancel){
          ...
          APP.showPushNotification(payload);
        }
      });
      dialog.show();
    }
