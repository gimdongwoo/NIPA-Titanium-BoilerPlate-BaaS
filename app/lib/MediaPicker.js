...
var NavigationBar = function(_title, _parent) {
	this.navBar = Alloy.createWidget("com.mcongrove.navigationBar", "widget", {
		id: "NavigationBar",
		__parentSymbol: _parent
	});
	this.navBar.setParent(_parent);
	this.navBar.setBackgroundColor(APP.Settings.colors.primary, APP.Settings.colors.secondary);
	this.navBar.setTitle(_title, APP.Settings.navBarStyle.titleStyle);

	return this;
};
NavigationBar.prototype.setBack = function(_clickFn) {
	this.navBar.showBack(function(_event) {
		_clickFn && _clickFn();
	}, APP.Settings.navBarStyle.backBtnStyle);

	return this;
};
NavigationBar.prototype.setRightEx = function(_text, _clickFn) {
	this.navBar.showRightEx({
		_textProperties : {
			text : _text,
			color : "#ff5757",
			textAlign: "center",
		...
		_callback : _.debounce(function(_event) {
			_clickFn && _clickFn();
		}, 700, true)
	});
...
exports.MediaPicker.prototype.show = function(cb, max, type, message) {
	...
	if (OS_ANDROID) {
		var win_assetGroups = new Element('Window', {title: _lang[type], navBarHidden: true, theme: "Theme.NoActionBar", backgroundColor: '#fff'}).element;
	} else {
		var win_assetGroups = new Element('Window', {title: _lang[type], navBarHidden: true, backgroundColor: '#fff'}).element;
	}
	...
	// NavigationBar
	new NavigationBar("사진 선택", win_assetGroups).setBack(function() { _nav.closeAll(); }).setRightEx("선택", doneFn);

	// var cancel = new Element('Button', {title: 'Cancel', bottom: 0,left: 0, zIndex: 99}).addClick(function() {
	// 	_nav.closeAll();
	// }).element;
	// if (OS_IOS) win_assetGroups.setLeftNavButton(cancel);
	...
	var gotAssetGroups = function(e) {
		...
		new Element('TableView', {
			...
			if (OS_ANDROID) {
				win_assets = new Element('Window', {title: e.rowData.caption, navBarHidden: true, theme: "Theme.NoActionBar"}).element;
			} else {
				win_assets = new Element('Window', {title: e.rowData.caption, navBarHidden: true}).element;
			}

			// NavigationBar
			new NavigationBar(e.rowData.caption, win_assets).setBack(function() { win_assets.close(); }).setRightEx("선택", doneFn);
		...
		win_assets.addEventListener('close', function() {
			...
			for (var i = 0; i < assets.length; i++) {
				for (var j = 0; j < assets[i].length; j++) {
					...
					if (!iv) continue;
					iv.image = null;
					if (OS_IOS) rows[iv.row].remove(iv);
					else grid.remove(iv);
					iv = null;
				}
			}
	...
	var loadImages = function(row, lzy) {
		...
		for (var i = 0; i < toAdd.length; i++) {
			...
			queue++;
			(function(iv, lzy) {
				MediaPickerModule.getThumb({url: OS_ANDROID&&type=='photos'?iv.id:iv.assetUrl, success: function(e) {
					queue--;
					if (!e.image) return;
	...
	function createIV(row, i) {
		...
		var iv = Ti.UI.createImageView({
			image: OS_IOS ? 'images/no_media.png' : null,
			top: OS_IOS ? 4 : 4+row*(_size+4),
		...
		iv.addEventListener('longpress', function(e) {
			...
			if (type == 'videos') {
				...
				var iv = e.source;
				if (OS_IOS) activeMovie.media = assets[iv.row][iv.i].url;
...
exports.MediaPicker.prototype.getImageByURL = function(opt) {
	if (OS_IOS) MediaPickerModule.getImageByURL(opt);
	else {
