...
var APP = {
	...
	initUser: function() {
		...
		APP.UserM = Alloy.Models.instance('User');
		...
		APP.UserM.on('change', function() {
			APP.log("debug", "User Cached : SettingsM.UserM");
			APP.SettingsM.save({ "UserM": JSON.stringify(APP.UserM.attributes) });
		});
	...
	initAfterLogin: function(isJoining) {
		...
		if (OS_IOS && APP.navigationWindow) {
	...
	*/
	...
	handleNavigation: _.debounce(function(_id, isForceOpen) {
		...
		if(_id == APP.currentStack) {
			...
			if(APP.Settings.useSlideMenu) {
				...
				APP.SlideMenu.setIndex(tabId);
			} else {
				// Move the tab selection indicator
				APP.Tabs.setIndex(tabId);
			}
	...
	addChild: function(_controller, _params, _modal, _sibling) {
		...
		// Ti.App.fireEvent('handleNavigation', { "name" : "" });
	...
	removeChild: function(_modal) {
		...
		if (stack.length <= 1) { Ti.App.fireEvent('handleNavigation', { "name" : hName }); }
		// nullout
	...
	*/
	...
	resumeObserver: function(_event) {
		...
		if (OS_ANDROID) {
			require("push").notificationClear();
		}
	},
	/**
