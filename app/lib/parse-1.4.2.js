...
(function(root) {
   ...
   *      Ti.API.debug("Updated " + result.id);
   *    });
   * </pre></p>
   *
  ...
  _.extend(Parse.Promise, /** @lends Parse.Promise */ {
     ...
     *     Ti.API.debug(r1);  // prints 1
     *     Ti.API.debug(r2);  // prints 2
     *     Ti.API.debug(r3);  // prints 3
     *   });</pre>
     *
...
(function(root) {
   ...
   *           Ti.API.debug("Couldn't delete " + error.errors[i].object.id +
   *             "due to " + error.errors[i].message);
   *         }
   *       } else {
   *         Ti.API.debug("Delete aborted because of " + error.message);
   *       }
   *     },
   *   });
   * </pre>
