...
//라벨에 특정텍스트 bold효과주기.
exports.effectLabelToBold = function(label, originText, boldTexts, fontSize) {
	var attributes = [];
	...
	for(var i=0,max=boldTexts.length; i<max; ++i){
		...
		var firstIdx = originText.indexOf(boldText);
		if(firstIdx != -1){
			attributes.push( {
				type: Titanium.UI.ATTRIBUTE_FONT,
				//ios에서 fontSize안주면 설정하지않은 기본값이됨.
				value: {fontSize:fontSize, fontWeight:'bold'},
...
/**
* listview find event fired item & block double action
*/
exports.listViewFindItem = _.debounce(function(e, doAction) {
	if(e.section){
